...
module ValidatesDateTime
  ...
  DEFAULT_TEMPORAL_VALIDATION_OPTIONS = {
    :before_message => "年齡必須在13歲以上",
    :after_message  => "年齡必須在82歲以下",
    :on => :save
  }.freeze
  ...
  module ClassMethods
    def validates_date(*args)
      options = temporal_validation_options({ :message => "並不是合法的日期" }, args)
      attr_names = args
    ...
    def validates_time(*args)
      options = temporal_validation_options({ :message => "並不是合法的時間格式" }, args)
      attr_names = args
    ...
    def validates_date_time(*args)
      options = temporal_validation_options({ :message => "並不是合法的日期格式" }, args)
      attr_names = args
