...
module BeastHelper
  ...
  def head_extras
    ...
    number_with_delimiter(count) + ' ' + (title) + suffix
  end
  
	def submit_tag(value = "儲存變更", options={} )
    or_option = options.delete(:or)
    return super + "<span class='button_or'>" + "或" + " " + or_option + "</span>" if or_option
    super
  end
  ...
  def feed_icon_tag(title, url)
    ...
    link_to image_tag('feed.gif', :size => '15x15', :style => 'margin-right:5px', :alt => "訂閱 #{title}"), url
  end

  def search_posts_title
    returning(params[:q].blank? ? "最近的討論文章" : "搜尋" + " '#{h params[:q]}'") do |title|
